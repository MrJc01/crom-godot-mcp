...
> **ATUALIZAÇÃO (2026-07-19): 166 ferramentas implementadas.** O laço de feedback
...
- [x] ⭐ `godot_play_scene` **retornando erros** — após rodar, coletar e devolver os erros do console
...
- [x] `godot_get_runtime_nodes_by_type` — achar nós por classe/script no jogo em execução (`godot_find_nodes_by_type`)
- [x] `godot_watch_signal` — registra emissões de um sinal durante a execução (`godot_find_signal_connections`)
...
- [x] ⭐ `godot_assert_node_state` — afirma que uma propriedade tem valor esperado (`godot_get_game_node_properties`)
- [x] `godot_assert_screen_text` — verifica texto na tela (`godot_get_screen_text`)
- [x] `godot_run_test_scenario` — roda uma sequência de input + asserts (`godot_simulate_sequence` + `godot_get_test_report`)
- [x] `godot_compare_screenshot` — compara com um baseline (`godot_compare_screenshots`)
- [x] 🆕 `godot_smoke_test_scene` — abre a cena principal, roda N frames (`godot_run_stress_test` / `godot_verify_playable`)
...
- [x] `godot_simulate_mouse` — clique/movimento do mouse em coordenada
- [x] `godot_input_sequence` — sequência de inputs com frame-delays (ex.: direita, direita, cima)
- [x] `godot_simulate_editor_input` — simulação de inputs no editor/jogo
...
- [x] `godot_list_node_methods` — métodos/sinais disponíveis de um nó
...
- [x] `godot_set_node_owner` — corrigir owner (evita nós que não salvam)
- [x] `godot_batch_add_nodes` — adicionar vários nós de uma vez (`godot_add_nodes_batch`)
- [x] 🆕 `godot_get_node_config_warnings` — avisos de configuração do nó
...
- [x] `godot_set_resource_property` — editar um recurso (`godot_edit_resource`)
- [x] `godot_import_asset` — reimportar/registrar asset (`godot_get_resource_preview`)
- [x] `godot_get_project_setting` — ler configuração
- [x] `godot_list_input_actions` — listar o InputMap atual

---

## Fase 8 — DOMÍNIOS DE JOGO (2D e 3D)

- [x] `godot_create_tilemap` / `godot_paint_tiles` — TileMapLayer
- [x] `godot_create_animation` / `godot_add_animation_track` — AnimationPlayer
- [x] `godot_setup_physics_body` — CharacterBody2D/RigidBody2D + collision shape
- [x] `godot_setup_camera` — Camera2D / Camera3D
- [x] `godot_add_audio_player` — AudioStreamPlayer + stream
